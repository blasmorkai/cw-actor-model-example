...
mod tests {
    ...
    fn create_counter_and_increment_twice() {
        ...
        let manager_contract = manager_instantiate(&mut app, manager_id); /// instantiates contract
    ...
    fn create_counter_and_increment_and_reset() {
        ...
        instantiate_new(&mut app, &manager_contract, counter_id); // instantiates manager contract
        increment(&mut app, &manager_contract, "contract1".to_string()); // adds 1 to counter (1)
    ...
    fn create_two_counters_and_increment_each() {
        ...
        //
        instantiate_new(&mut app, &manager_contract1, counter_id); // inst Counter 1
        instantiate_new(&mut app, &manager_contract2, counter_id); // inst Counter 2

        let res = get_contracts(&app, &manager_contract1);
        assert_eq!(res.contracts.len(), 1);

        print!(">>{}\n", manager_contract1.addr());
        print!(">>{:?}\n", res.contracts);
        let res = get_contracts(&app, &manager_contract2);
        assert_eq!(res.contracts.len(), 1);

        print!(">>{}\n", manager_contract2.addr());
        print!(">>{:?}\n", res.contracts);
