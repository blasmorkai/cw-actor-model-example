...
fn handle_instantiate_reply(deps: DepsMut, msg: Reply) -> StdResult<Response> {
    // When we instantiate, we can use parse_reply_instantiate_data to get the contract address
    //struct MsgInstantiateContractResponse {contract_address: String, data: Option<Binary>,}      
    //println!("{:?}", msg.clone());
...
pub fn instantiate_new_counter(
    ...
    // In case we want to set up a gas limit
...
// struct Event {ty: String, attributes: Vec<Attribute>} ty is type, type is reserved in Rust. 
// struct Attribute {key: String, value: String,}
// the msg: &Reply is meant to be the reply from an execute message.

fn get_contract_address(msg: &Reply) -> String {
    println!(">>>>>>>>>>>>>>>>>> {:?}\n\n", msg.result.clone());

    let result:String = msg.result.clone().unwrap()
    .events.iter()
    ...
    let result:String = msg.result.clone().unwrap()
    .events.iter()
    .filter(|event| event.ty == "wasm" && event.attributes[0].key.contains("_contract_addr"))
    .map(|p| p.attributes[0].value.clone())
    .collect();
...
fn get_reset_count(msg: &Reply) -> i32 {
    let result :String = msg.result.clone().unwrap()
    .events.iter()
    .filter(|event| event.ty == "wasm" && event.attributes.len() == 3 && event.attributes[1].value == "reset")
